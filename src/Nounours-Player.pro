...
win32 {
    ...
    QMAKE_TARGET_PRODUCT += Nounours Player
    QMAKE_TARGET_DESCRIPTION += Nounours Player
    #RC_LANG +=
...
isEmpty(NOUNOURSDIR):NOUNOURSDIR=/usr/share/nounours-player

target.path = $$INSTROOT$$BINDIR
logo.path = $$INSTROOT$$MEDIADIR
icon.path = $$INSTROOT$$ICONDIR
desktop.path = $$INSTROOT$$APPDIR
manual.path = $$INSTROOT$$DOCDIR/nounours-player
man.path = $$INSTROOT$$MANDIR/man1
license.path = $$INSTROOT$$LICENSEDIR/nounours-player
translations.path = $$INSTROOT$$NOUNOURSDIR/translations

logo.files = ../etc/logo/nounours-player.svg
icon.files = ../etc/logo/nounours-player.svg
desktop.files = ../etc/nounours-player.desktop
manual.files = ../DOCS/nounours-player.md
man.files = ../DOCS/nounours-player.1.gz
...
CONFIG(embed_translations) {
    ...
    NOUNOURS_LANG_PATH += :/translations
...
CONFIG(install_translations) {
    ...
    NOUNOURS_LANG_PATH += $$NOUNOURSDIR/translations
...
isEmpty(SETTINGS_FILE):SETTINGS_FILE=nounoursplayer
DEFINES += "NOUNOURS_PLAYER_VERSION=\\\"$$VERSION\\\"" \
           "SETTINGS_FILE=\\\"$$SETTINGS_FILE\\\"" \
           "NOUNOURS_PLAYER_LANG_PATH=\\\"$$NOUNOURS_LANG_PATH\\\""
!isEmpty(NOUNOURS_LANG):DEFINES += "NOUNOURS_PLAYER_LANG=\\\"$$NOUNOURS_LANG\\\""
