...
bool GestureHandler::Begin(int gesture_type, QPoint mousePos, QPoint windowPos)
{
    if(!elapsedTimer)
    {
        ...
        hRatio = screen ? screen->physicalDotsPerInchX() / 800.0 : 1.0;
        vRatio = screen ? screen->physicalDotsPerInchY() / 450.0 : 1.0;
    }
    else
        return false;
    ...
    else // if(gesture_type == HSEEK_VVOLUME)
    {
        ...
        start.time = nounours->mpv->getTime();
        start.volume = nounours->mpv->getVolume();
    }
...
bool GestureHandler::Process(QPoint mousePos)
{
    if(elapsedTimer && elapsedTimer->elapsed() > timer_threshold) // 10ms seems pretty good for all purposes
    {
        ...
        if(gesture_type == MOVE)
            nounours->window->move(start.windowPos + delta);
        else
        {
            ...
            case SEEKING:
            {
                ...
                nounours->mpv->Seek(start.time + relative, false, true);
                break;
            }
            case ADJUSTING_VOLUME:
                nounours->mpv->Volume(start.volume - delta.y() * vRatio, true);
                break;
            }
        }
