...
NounoursEngine::NounoursEngine(QObject *parent):
    QObject(parent),
...
void NounoursEngine::LoadSettings()
{
    settings->Load();
    Load2_0_3();
}

void NounoursEngine::Command(QString command)
{
    if(command == QString())
        return;
    ...
    if(!args.empty())
    {
        if(args.front() == "nounours") // implicitly understood
            args.pop_front();

        if(!args.empty())
        {
            auto iter = NounoursCommandMap.find(args.front());
            if(iter != NounoursCommandMap.end())
            {
                args.pop_front();
            ...
            RequiresParameters("nounours");
    }
    else
        InvalidCommand(args.join(' '));
}

void NounoursEngine::Print(QString what, QString who)
{
...
void NounoursEngine::PrintLn(QString what, QString who)
{
    Print(what+"\n", who);
}

void NounoursEngine::InvalidCommand(QString command)
{
    PrintLn(tr("invalid command '%0'").arg(command));
}

void NounoursEngine::InvalidParameter(QString parameter)
{
    PrintLn(tr("invalid parameter '%0'").arg(parameter));
}

void NounoursEngine::RequiresParameters(QString what)
{
